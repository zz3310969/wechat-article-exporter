...
async function newLoginSession() {
  ...
  const resp = await $fetch<StartLoginResult>(`/api/v1/login/session/${sid}`, { method: 'POST' });
...
async function getQrcode() {
  try {
    ...
    qrcodeSrc.value = `/api/v1/login/getqrcode?rnd=${Math.random()}`;
    msg.value = '';
...
async function checkQrcodeStatus() {
  const resp = await $fetch<ScanLoginResult>('/api/v1/login/scan', {
    method: 'GET',
  });
  if (resp && resp.base_resp && resp.base_resp.ret === 0) {
    switch (resp.status) {
...
async function bizLogin() {
  try {
    loading.value = true;
    const resp = await $fetch<LoginAccount>('/api/v1/login/login', {
      method: 'POST',
    });
    if (resp.err) {
    ...
    msg.value = '授权成功';
    preferences.value.api_auth = resp;
  } catch (e: any) {
    msg.value = e.message;
  } finally {
    loading.value = false;
...
<template>
  <UModal prevent-close>
    <UCard>
      <template #header>
        <h2 class="text-lg font-semibold">获取 API 授权</h2>
